...
import pandas as pd
random_user = int(pd.Series(user_movie_df.index).sample(1, random_state=45).values.item())
# kullanıcılardan rastgele bir örneklem çekiyoruz.
...
# veri seti random user'a indirgenmiş olacak. yani index'te user, sütunlarda tüm filmler

movies_watched = random_user_df.columns[random_user_df.notna().any()].tolist()
# NaN dışındakileri getirdik. random user için
...
# 33 film için bütün userlara göre veris seti oluşturduk.

user_movie_count = movies_watched_df.T.notnull().sum()
# transpozunu aldık ve NaN olmayanların(dolu) toplamını aldık

user_movie_count = user_movie_count.reset_index()
#bütün kullanıcıların 33 filmden kaç tanesini izlediği

user_movie_count.columns = ["userId", "movie_count"]

user_movie_count[user_movie_count["movie_count"] > 20].sort_values("movie_count", ascending=False)
# user la en az aynı 20 filmi izlemiş kullanıcılar

user_movie_count[user_movie_count["movie_count"] == 33].count()
# user'ın izlediği bütün filmleri izleyen kullanıcıların sayısı


users_same_movies = user_movie_count[user_movie_count["movie_count"] > 20]["userId"]
# user'la aynı 20 filmi izlemiş kullanıcılaın id'leri
...
def create_user_movie_df():
    ...
    movie = pd.read_csv('Miuul/5. Hafta (Tavsiye Sistemleri)/recommender_systems/datasets/movie_lens_dataset/movie.csv')
    rating = pd.read_csv(
        'Miuul/5. Hafta (Tavsiye Sistemleri)/recommender_systems/datasets/movie_lens_dataset/rating.csv')
    df = movie.merge(rating, how="left", on="movieId")
    comment_counts = pd.DataFrame(df["title"].value_counts())
    rare_movies = comment_counts[comment_counts["count"] <= 1000].index
    common_movies = df[~df["title"].isin(rare_movies)]
...
random_user = int(pd.Series(user_movie_df.index).sample(1).values.item())
user_based_recommender(random_user, user_movie_df, cor_th=0.90, score=4.5)
