...
populasyon = np.random.randint(0, 80, 10000) # 0 ile 80 arasında 10000 tane sayı
populasyon.mean()

np.random.seed(115) # aynı random sayıları elde etmek için 115=seed value
...
#Parametric Ortalama
#Tanım: Parametrik ortalama, verilerin belirli bir dağılımı (genellikle normal dağılım) takip ettiğini varsayan
#Örnekler: Aritmetik ortalama, varyans ve standart sapma.
#Varsayımlar: Verilerin normal dağılım gösterdiği ve örneklemin yeterince büyük olduğu varsayılır



#Non-Parametric Ortalama
#Tanım: Non-parametrik ortalama, verilerin belirli bir dağılımı takip etmesi gerekmeyen ve
# dağılımın şeklinden bağımsız olarak hesaplanan ortalamadır.
# Örnekler: Medyan, mod, sıralı ortalama.
# Varsayımlar: Veriler herhangi bir belirli dağılımı takip etmek zorunda değildir
# ve küçük örneklemlerle çalışılabilir.
...
sms.DescrStatsW(df["total_bill"]).tconfint_mean()  #ilgili değişken için güven aralığı %95 güven
sms.DescrStatsW(df["tip"]).tconfint_mean()

# Titanic Veri Setindeki Sayısal Değişkenler için Güven Aralığı Hesabı
df = sns.load_dataset("titanic")
df.describe().T
sms.DescrStatsW(df["age"].dropna()).tconfint_mean() # eksik değerleri önce uçurduk
...
# corr = 0.57 pozitif yönlü orta şiddette bir ilişki vardır.
...
#   - 2. Varyans Homojenliği  2 grubun dağılımlarının benzerliği
# 3. Hipotezin Uygulanması
#   - 1. Varsayımlar sağlanıyorsa bağımsız iki örneklem t testi (parametrik test)
#   - 2. Varsayımlar sağlanmıyorsa mannwhitneyu testi (non-parametrik test)
# 4. p-value değerine göre sonuçları yorumla
# Not:
# - Normallik sağlanmıyorsa direk 3'ün 2si numara. Varyans homojenliği sağlanmıyorsa 3'ün 1i numaraya arguman girilir.
...
# Test Stat = 0.9367, p-value = 0.0002 ho red
...
test_stat, pvalue = shapiro(df.loc[df["smoker"] == "No", "total_bill"])  # normallik testi
print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue))
# Test Stat = 0.9045, p-value = 0.0000  ho red
...
print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue))
# levene testi 2 farklı grup gönderilir varyans homojenliğinin sağlanıp sağlanmadığı test edilir.
# Test Stat = 4.0537, p-value = 0.0452  ho red
...
# 2 varsayımın sağlandığını düşündük
test_stat, pvalue = ttest_ind(df.loc[df["smoker"] == "Yes", "total_bill"],
                              df.loc[df["smoker"] == "No", "total_bill"],
                              equal_var=True)  #normallik sağlandı varyans sağlanmazsa equal_var=False denecek

print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue))
# Test Stat = 1.3384, p-value = 0.1820 ho reddedilemz(fark yoktur)
# ödenen hesaplar arasında istatistiki olarak anlamlı bir fark yoktur.
...
# Test Stat = 7531.5000, p-value = 0.3413  ho reddedilemez
...
print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue))
# normallik varsayımı sağlanmadığı için direkt nonparamtrice gidilmeli
...
print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue))
# Test Stat = 53212.5000, p-value = 0.0261  ho reddedilir (fark var)
...
df = pd.read_csv("Miuul/4. Hafta/diabetes.csv")
df.head()
...
# Test Stat = 0.8012, p-value = 0.0000     ho red direkt manwhitneyu



# Normallik varsayımı sağlanmadığı için nonparametrik.

# Hipotez (H0: M1 = M2)  #medyanların kıyaslaması
test_stat, pvalue = mannwhitneyu(df.loc[df["Outcome"] == 1, "Age"].dropna(),
                                 df.loc[df["Outcome"] == 0, "Age"].dropna())
print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue))
#ho reddedilir. daha yüksek yaşlar diabete sahip gibi
...
df = pd.read_csv("Miuul/4. Hafta/course_reviews.csv")
df.head()
...
print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue))
# Test Stat = 0.5710, p-value = 0.0000  ho red
...
# kursu izleyenler daha fazla rating veriyor.
...
print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue))
# ho reddedilemez. varyanslar homojen ama normallikten düştük
...
#gruplar arasında fark var.

from statsmodels.stats.multicomp import MultiComparison
comparison = MultiComparison(df['total_bill'], df['day'])
tukey = comparison.tukeyhsd(0.05)
print(tukey.summary())
# 2'li karşılaşma yapıldığında günler arasında bir fark yok.
